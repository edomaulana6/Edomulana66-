...
def run_pre_flight_checks():
    ...
    if not (os.getenv("TELEGRAM_TOKEN") and os.getenv("TELEGRAM_TOKEN") != "GANT
I_DENGAN_TOKEN_ANDA"):
...
:
    ...
    if format_choice == 'audio':
        ydl_opts.update({'format': 'bestaudio/best', 'postprocessors': [{'key':
'FFmpegExtractAudio', 'preferredcodec': 'mp3'}]})
    ...
    try:
        ...
        path = os.path.splitext(base_path)[0] + '.mp3' if format_choice == 'audi
o' else base_path
        if not os.path.exists(path): raise FileNotFoundError(f"File not found: {
path}")

        filename = f"{re.sub(r'[\\/*?:\"<>|]', '', info.get('title', 'media'))}.
{'mp3' if format_choice == 'audio' else info.get('ext', 'mp4')}"

        with open(path, 'rb') as f:
            if format_choice == 'audio':
                await effective_message.reply_audio(f, title=info.get('title'),
filename=filename, caption=info.get('title'))
            else:
                await effective_message.reply_video(f, filename=filename, captio
n=info.get('title'))
    except Exception as e:
        logger.error(f"Download failed for '{identifier}': {e}", exc_info=True)
        await effective_message.reply_text(f"❌ Gagal: Link tidak didukung atau e
rror internal.")
    finally:
...
async def start(update: Update, context: CallbackContext):
    ...
    await update.message.reply_html(f"👋 Halo {update.effective_user.mention_html
()}! /help untuk bantuan.")
...
async def download_get_url(update: Update, context: CallbackContext):
    ...
    try:
        with yt_dlp.YoutubeDL({'quiet': True, 'noplaylist': True}) as ydl: info
= ydl.extract_info(url, download=False)
        context.user_data['url_info'] = {'url': url, 'title': info.get('title',
'Tanpa Judul')}
        keyboard = [[InlineKeyboardButton("🎧 Audio", callback_data=f"dl_url:audi
o"), InlineKeyboardButton("🎬 Video", callback_data=f"dl_url:video")]]
        await update.message.reply_photo(photo=info.get('thumbnail'), caption=f"
Pilih format untuk:\n*{info.get('title')}*", reply_markup=InlineKeyboardMarkup(k
eyboard), parse_mode='Markdown')
    except Exception:
        await download_media(url, 'video', update.message)
    return CHOOSE_FORMAT

async def _execute_and_send_search(chat_id, context: CallbackContext, is_new_sea
rch=False):
    query = context.user_data.get('search_query')
    status_message = None
    try:
        if is_new_search:
            status_message = await context.bot.send_message(chat_id=chat_id, tex
t="⏳ Mencari 250 video...")
            search_query = f"ytsearch250:{query}"
            ydl_opts = {'quiet': True, 'noplaylist': True, 'match_filter': 'dura
tion < 600', 'extract_flat': True}
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not filtered_entries:
                await context.bot.send_message(chat_id=chat_id, text="Tidak ada
hasil yang cocok dengan filter durasi (di bawah 10 menit).")
        ...
        await context.bot.send_message(chat_id=chat_id, text=f"❌ Gagal: Terjadi
error saat melakukan pencarian mendalam.")
    finally:
        if status_message: await status_message.delete()
...
async def _display_search_page(update: Update, context: CallbackContext):
    ...
    keyboard = []
    # Membuat 2 kolom tombol untuk setiap lagu
    for i in range(0, len(page_results), 2):
        row = []
        # Lagu pertama dalam baris
        video_id1 = page_results[i]['id']
        row.extend([
            InlineKeyboardButton(f"🎧 {i+1}", callback_data=f"dl_search:audio:{video_id1}"),
            InlineKeyboardButton(f"🎬 {i+1}", callback_data=f"dl_search:video:{video_id1}")
        ])
        # Jika ada lagu kedua untuk baris ini
        if i + 1 < len(page_results):
            video_id2 = page_results[i+1]['id']
            row.extend([
                InlineKeyboardButton(f"🎧 {i+2}", callback_data=f"dl_search:audio:{video_id2}"),
                InlineKeyboardButton(f"🎬 {i+2}", callback_data=f"dl_search:video:{video_id2}")
            ])
        keyboard.append(row)
...
async def song_get_title(update: Update, context: CallbackContext):
    ...
    try:
        ...
        ydl_opts = {'quiet': True, 'noplaylist': True, 'match_filter': 'duration
 < 600'}
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            result = ydl.extract_info(search_query, download=False)
            if not result.get('entries'):
                await update.message.reply_text("Tidak ada lagu yang cocok denga
n filter durasi.")
                if status_message: await status_message.delete()
                return ConversationHandler.END
        ...
        logger.error(f"Song search failed for '{update.message.text}': {e}", exc
_info=True)
        if status_message: await status_message.delete()
        await update.message.reply_text(f"❌ Gagal: Terjadi error saat mencari la
gu.")
...
async def get_photo(update: Update, context: CallbackContext):
    file = await (update.message.photo[-1] if update.message.photo else update.m
essage.document).get_file()
    os.makedirs('downloads', exist_ok=True)
    path = os.path.join('downloads', f"{uuid.uuid4()}.jpg")
    await file.download_to_drive(path)
    context.user_data['photo_path'] = path
    keyboard = [[InlineKeyboardButton("Tajamkan", callback_data="enhance:sharpen
"), InlineKeyboardButton("Kontras", callback_data="enhance:contrast")]]
    await update.message.reply_text("Pilih peningkatan:", reply_markup=InlineKey
boardMarkup(keyboard))
...
async def get_video(update: Update, context: CallbackContext):
    ...
    keyboard = [
        [InlineKeyboardButton("✨ Tingkatkan Kualitas", callback_data="convert:en
hance_quality")],
        [InlineKeyboardButton("4k", callback_data="convert:4k"), InlineKeyboardB
utton("2k", callback_data="convert:2k")],
        [InlineKeyboardButton("1080p", callback_data="convert:1080p"), InlineKey
boardButton("720p", callback_data="convert:720p")]
    ]
    await update.message.reply_text("Pilih tindakan:", reply_markup=InlineKeyboa
rdMarkup(keyboard))
...
async def download_callback_handler(update: Update, context: CallbackContext):
    ...
    if not entry:
        await query.message.reply_text("Error: Hasil tidak ditemukan atau sesi k
edaluwarsa.")
    ...
    await query.message.reply_text(f"⏳ Memulai unduhan untuk *{entry.get('title'
)}*...", parse_mode='Markdown')
    await download_media(identifier, action, query.message)

    # Tidak mengakhiri conversation, agar bisa unduh yang lain dari halaman yang
 sama
    return CHOOSE_FORMAT
...
def main():
    ...
    app = Application.builder().token(os.getenv("TELEGRAM_TOKEN")).persistence(p
ersistence).post_init(post_init).build()

    conv_defaults = {'allow_reentry': True, 'conversation_timeout': 300, 'fallba
cks': [CommandHandler("cancel", cancel)]}

    app.add_handler(ConversationHandler(
        entry_points=[CommandHandler("download", download_start)],
        states={GET_URL: [MessageHandler(filters.TEXT & ~filters.COMMAND, downlo
ad_get_url)], CHOOSE_FORMAT: [CallbackQueryHandler(download_callback_handler, pa
ttern="^dl_url:")]},
        **conv_defaults
    ))
    app.add_handler(ConversationHandler(
    ...
    app.add_handler(ConversationHandler(entry_points=[CommandHandler("song", son
g_start)], states={GET_SONG_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMA
ND, song_get_title)]}, **conv_defaults))
    app.add_handler(ConversationHandler(entry_points=[CommandHandler("enhance_ph
oto", enhance_photo_start)], states={GET_PHOTO: [MessageHandler(filters.PHOTO |
filters.Document.IMAGE, get_photo)], GET_ENHANCEMENT: [CallbackQueryHandler(phot
o_enhancement_handler, pattern="^enhance:")]}, **conv_defaults))
    app.add_handler(ConversationHandler(
        entry_points=[CommandHandler("convert_video", convert_video_start)],
        states={
            GET_VIDEO: [MessageHandler(filters.VIDEO | filters.Document.VIDEO, g
et_video)],
            GET_PROCESS_ACTION: [CallbackQueryHandler(video_processing_handler,
pattern="^convert:")]
        },
        **conv_defaults
    ))
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, collect_user
_id))
